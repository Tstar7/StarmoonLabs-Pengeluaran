...
function bindEvents(){
  ...
  els.monthFilter.addEventListener('change', () => {
    els.budgetInput.value = formatPlainNumber(budgets[getMonthKey()] || '')
    renderAll()
  })

  els.typeInput.addEventListener('change', populateCategorySelect)
  els.amountInput.addEventListener('input', () => formatCurrencyInput(els.amountInput))
  els.budgetInput.addEventListener('input', () => formatCurrencyInput(els.budgetInput))
...
function handleSubmit(event){
  ...
  const item = {
    ...
    amount: parseFormattedNumber(els.amountInput.value),
...
function editTransaction(id){
  ...
  els.amountInput.value = formatPlainNumber(item.amount)
...
async function deleteTransaction(id){
  ...
  const ok = await sweetConfirm({
    ...
    text: `Transaksi "${item.name}" akan dihapus dari perangkat ini.`,
    icon: 'warning',
...
function saveBudget(){
  const value = parseFormattedNumber(els.budgetInput.value)
...
function renderAll(){
  ...
  els.budgetInput.value = formatPlainNumber(budgets[monthKey] || '')
  renderSummary()
...
function renderCharts(){
  ...
  drawPieChart(document.getElementById('pieChart'), categoryData, 'Belum ada data kategori', els.pieLegend)
...
function drawPieChart(canvas, data, emptyText, legendEl){
  const ctx = setupCanvas(canvas)
  if(!ctx) return
  const { width, height } = canvas
  clearCanvas(ctx, width, height)
  ...
  if(!data.length){
    drawEmpty(ctx, width, height, emptyText)
    if(legendEl){
      legendEl.innerHTML = '<p class="pie-legend-empty">Belum ada kategori.</p>'
    }
    return
  }

  const shownData = data.slice(0, 10)
  const total = sumAmount(shownData.map(item => ({ amount: item.value })))
  const cx = width / 2
  const cy = height / 2
  ...
  ctx.fillStyle = getTextColor()
  ctx.font = width < 210 ? '800 12px system-ui' : '800 15px system-ui'
  ctx.textAlign = 'center'
  ctx.fillText(shortCurrency(total), cx, cy + 5)
  ctx.textAlign = 'left'

  if(legendEl){
    legendEl.innerHTML = shownData.map((item, index) => {
      ...
      return `
        <div class="pie-legend-item">
          ...
          <div class="pie-legend-text">
            ...
            <span>${percent}% • ${escapeHtml(shortCurrency(item.value))}</span>
          </div>
        </div>
      `
    }).join('')
...
function setupCanvas(canvas){
  ...
  const minWidth = canvas.id === 'pieChart' ? 150 : 300
  ...
  try{
    ...
    if(mode === 'replace'){
      const ok = await sweetConfirm({
        ...
        text: 'Semua transaksi dan budget lama akan diganti dengan file backup ini.',
        icon: 'warning',
...
async function fillSampleData(){
  const ok = await sweetConfirm({
    title: 'Tambahkan data contoh?',
    text: 'Data contoh akan ditambahkan ke bulan yang sedang dipilih.',
    icon: 'question',
...
async function resetAllData(){
  const ok = await sweetConfirm({
    ...
    text: 'Semua transaksi dan budget akan dihapus. Data tidak bisa dikembalikan kecuali kamu punya backup.',
    icon: 'warning',
...
function formatCurrencyInput(input){
  ...
  input.value = formatPlainNumber(input.value)
...
function sweetConfirm({ title, text = '', icon = 'question', confirmText = 'Ya', cancelText = 'Batal', danger = false } = {}){
  return new Promise(resolve => {
    const root = getSweetAlertRoot()
    const iconLabel = getSweetIcon(icon)
    root.innerHTML = `
      <div class="sweet-backdrop" data-sweet-close="cancel"></div>
      <div class="sweet-card" role="dialog" aria-modal="true" aria-labelledby="sweetTitle">
        <div class="sweet-icon ${escapeHtml(icon)}">${iconLabel}</div>
        <h3 id="sweetTitle">${escapeHtml(title || 'Konfirmasi')}</h3>
        ${text ? `<p>${escapeHtml(text)}</p>` : ''}
        <div class="sweet-actions">
          <button class="btn ghost" type="button" data-sweet-close="cancel">${escapeHtml(cancelText)}</button>
          <button class="btn ${danger ? 'danger' : 'primary'}" type="button" data-sweet-close="confirm">${escapeHtml(confirmText)}</button>
        </div>
      </div>
    `
    ...
    root.querySelectorAll('[data-sweet-close]').forEach(button => {
      button.addEventListener('click', () => finish(button.dataset.sweetClose === 'confirm'), { once: true })
    })
  })
}

function getSweetAlertRoot(){
  let root = document.getElementById('sweetAlertRoot')
  if(!root){
    root = document.createElement('div')
    root.id = 'sweetAlertRoot'
    root.className = 'sweet-root'
    document.body.appendChild(root)
  }
  return root
}

function getSweetIcon(icon){
  const icons = {
    success: '✓',
    warning: '!',
    danger: '!',
